...
import {
  ...
  Loader2, AlertTriangle, Moon, Sun, Info, X, ImageIcon, ChevronDown, ChevronRight,
} from "lucide-react";
    ...
    <div className="relative inline-flex items-center justify-center shrink-0" style={{ width: size, height: size }}>
      {existing ? (
        ...
        <button onClick={() => inputRef.current?.click()}
          className="w-full h-full flex items-center justify-center rounded border border-dashed border-border hover:border-primary/50 hover:bg-muted/30 transition-colors text-muted-foreground" title="Upload icon">
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="bg-muted/60 border-b border-border">
                      ...
                      <th className="text-left px-2 py-2 font-medium text-muted-foreground whitespace-nowrap">
                        <button onClick={() => handleSort("slot")} className="flex items-center gap-1 hover:text-foreground transition-colors">
                          Slot <SortIcon col="slot" />
                        </button>
                      </th>
                      <th className="text-left px-2 py-2 font-medium text-muted-foreground whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          <button onClick={() => handleSort("studioLevel")} className="flex items-center gap-1 hover:text-foreground transition-colors">
                            Studio Lv <SortIcon col="studioLevel" />
                          </button>
                          <span className="text-muted-foreground/40">/</span>
                          <button onClick={() => handleSort("intReq")} className="flex items-center gap-1 hover:text-foreground transition-colors">
                            INT Req <SortIcon col="intReq" />
                          </button>
                        </div>
                      </th>
                      {STAT_ORDER.map((stat) => (
                        <th key={stat} className="text-center px-1.5 py-2 font-medium text-muted-foreground">
                          <button onClick={() => handleSort(stat)} className="flex flex-col items-center gap-0.5 hover:text-foreground transition-colors w-full">
                            {statIcons[stat]
                              ? <img src={statIcons[stat]} alt={stat} className="w-4 h-4 object-contain mx-auto" title={stat} />
                  ...
                  <tbody>
                    {filtered.length === 0 && (
                      <tr><td colSpan={colCount} className="text-center py-10 text-sm text-muted-foreground">No equipment found.</td></tr>
                    )}
                    {filtered.map((item) => {
                          ...
                          <tr className={`border-b border-border transition-colors ${isExpanded ? "bg-primary/5" : "hover:bg-muted/20"}`}>
                            ...
                            <td className="px-2 py-1.5 whitespace-nowrap">
                              <button
                                ...
                                className="flex items-center gap-1.5 font-medium text-left hover:text-primary transition-colors group"
                                title="Click to edit base & increment values"
                              >
                                {isExpanded
                            ...
                            <td className="px-2 py-1.5">
                              <Input
                                type="number" min={1} max={99} value={level}
                            ...
                            {STAT_ORDER.map((stat) => {
                              ...
                              return (
                                <td key={stat} className={`px-1.5 py-1.5 text-center text-xs tabular-nums ${val === 0 ? "text-muted-foreground/30" : "font-medium text-foreground"}`}>
                                  {val > 0 ? val : "—"}
                                </td>
                              );
                            })}
                                      ...
                                      <div key={stat} className="flex flex-col gap-0.5">
                                        ...
                                        <div className="flex gap-1">
                                          ...
                                          <div className="flex-1">
                                            ...
                                            <Input
                                              type="number" value={inc} step={0.1}
                      ...
                      <Card key={slot} className="border-border shadow-none">
                        <CardHeader className="pb-1 pt-3 px-3">
                          <div className="flex items-center justify-between">
                            ...
                            {entry.itemName && (
                              <button onClick={() => setLoadoutEntry(slot, { itemName: "" })} className="text-muted-foreground hover:text-destructive">
                                <X className="w-3 h-3" />
                              </button>
                            )}
                          </div>
                          ...
                          {item && entry.itemName && (
                            <div className="text-[10px] text-muted-foreground space-y-0.5 mt-1">
                              {STAT_ORDER.filter((s) => {
                                ...
                                const inc = getEffectiveStat(item, s, "inc", overrides);
                                return statAtLevel(b, inc, entry.level) > 0;
                              }).map((s) => {
                                const b = getEffectiveStat(item, s, "base", overrides);
                                const inc = getEffectiveStat(item, s, "inc", overrides);
                                return (
