// Export the map placement returned-object and validation layer for Kingdom Adventures.
//
// This script is intended to run inside Ghidra with Java scripting enabled.
// It resolves target RVAs and names against the loaded program image base before exporting.
// It writes seed resolution, callgraph, index, relevance, and returned-object note reports.
...
public class ExportMapPlacementReturnedObjectLayer extends GhidraScript {

    private static final String DEFAULT_OUTPUT_DIR =
        "C:/Users/anisb/OneDrive/Desktop/replit kingdom adventures - Copy/KA-Website/Reverse engineering/exports/active/map-placement-returned-object-layer";

    private static final TargetSpec[] TARGETS = new TargetSpec[] {
        new TargetSpec("FUN_020cc7a8", 0L),
    ...
    public void run() throws Exception {
        ...
        File returnedObjectFile = new File(outDir, "returned_object_notes.tsv");
        ...
        writeReturnedObjectNotes(returnedObjectFile, selectedFunctions);
    ...
    private boolean shouldIncludeDepth2(Function f) {
        return computeRelevanceScore(f) > 0 || isLikelyPlacementHelper(f) || callsCoreReturnedObjectSeed(f);
    }

    private boolean callsCoreReturnedObjectSeed(Function f) {
        ...
        for (Instruction instr = listing.getInstructionAt(f.getEntryPoint()); instr != null && f.getBody().contains(instr.getAddress()); instr = instr.getNext()) {
            for (Reference ref : instr.getReferencesFrom()) {
                ...
                if (target != null) {
                    ...
                    if (name.equals("FUN_020cc7a8") || name.equals("FUN_00a23e84") || name.equals("thunk_FUN_00a225b8") || name.equals("FUN_0208e2a4")) {
                        return true;
                    }
                }
            }
    ...
    private void writeReturnedObjectNotes(File outFile, Set<Function> functions) throws IOException {
        List<String> lines = new ArrayList<String>();
        lines.add("function|entrypoint|note");
        for (Function f : functions) {
            String note = collectReturnedObjectNotes(f);
            if (note.length() == 0) {
                note = "none";
            }
            lines.add(f.getName() + "@" + f.getEntryPoint().toString() + "|" + sanitizeString(note));
        }
        writeFile(outFile, join(lines, "\n") + "\n");
    }

    private String collectReturnedObjectNotes(Function f) {
        ...
        for (Instruction instr = listing.getInstructionAt(f.getEntryPoint()); instr != null && f.getBody().contains(instr.getAddress()); instr = instr.getNext()) {
            for (Reference ref : instr.getReferencesFrom()) {
                ...
                String name = target.getName();
                if (name.equals("FUN_020cc7a8")) {
                    reasons.append("validates=FUN_020cc7a8;");
                }
                else if (name.equals("FUN_00a23e84")) {
                    reasons.append("extracts=FUN_00a23e84;");
                }
                else if (name.equals("thunk_FUN_00a225b8")) {
                    reasons.append("global_object=thunk_FUN_00a225b8;");
                }
                else if (name.equals("FUN_0208e2a4")) {
                    reasons.append("returned_object=FUN_0208e2a4;");
                }
                else if (name.contains("FUN_0208e2a4") || name.contains("FUN_020cc7a8") || name.contains("FUN_00a23e84") || name.contains("thunk_FUN_00a225b8")) {
                    reasons.append("related_call=" + name + ";");
                }
            }
        }
    ...
    private void writeFailureReport(File outDir, List<TargetResolution> resolutions) throws IOException {
        ...
        builder.append("Target resolution failed for returned-object placement export\n");
