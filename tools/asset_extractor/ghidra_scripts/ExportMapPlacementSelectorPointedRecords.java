// Export selector-pointed records for Kingdom Adventures map placement.
//
// This script is intended to run inside Ghidra with Java scripting enabled.
// It resolves two pointed placement descriptor records and exports raw memory,
// xrefs, pointer walk details, nearby symbols, and metadata candidates.
...
public class ExportMapPlacementSelectorPointedRecords extends GhidraScript {

    private static final String DEFAULT_OUTPUT_DIR =
        "C:/Users/anisb/OneDrive/Desktop/replit kingdom adventures - Copy/KA-Website/Reverse engineering/exports/active/map-placement-selector-pointed-records";
    ...
    public void run() throws Exception {
        ...
        File pointedRecordsFile = new File(outDir, "pointed_records.tsv");
        File xrefFile = new File(outDir, "record_xrefs.tsv");
        File nearbySymbolsFile = new File(outDir, "nearby_record_symbols.tsv");
        File pointerWalkFile = new File(outDir, "record_pointer_walk.tsv");
        ...
        writePointedRecords(pointedRecordsFile, resolutions);
        writeRecordXrefs(xrefFile, resolutions);
        writeNearbyRecordSymbols(nearbySymbolsFile, resolutions);
        writeRecordPointerWalk(pointerWalkFile, resolutions);
        writeMetadataCandidates(metadataFile, resolutions);
        writeRecordMemoryFiles(outDir, resolutions);
    ...
    private void writePointedRecords(File outFile, List<TargetResolution> resolutions) throws IOException {
    ...
    private void writeRecordXrefs(File outFile, List<TargetResolution> resolutions) throws IOException {
        List<String> lines = new ArrayList<String>();
    ...
    private void writeNearbyRecordSymbols(File outFile, List<TargetResolution> resolutions) throws IOException {
        List<String> lines = new ArrayList<String>();
    ...
    private void writeRecordPointerWalk(File outFile, List<TargetResolution> resolutions) throws IOException {
        List<String> lines = new ArrayList<String>();
        lines.add("target_name|record_address|offset|pointer_value|pointer_address|pointer_symbol|pointer_type|classification|notes");
        ...
        for (TargetResolution res : resolutions) {
            ...
            for (int offset = 0; offset + pointerSize <= POINTER_SCAN_SIZE; offset += pointerSize) {
                Address probe = baseAddress.add(offset);
                if (!isAddressInMemory(probe)) {
                    continue;
                }
                ...
                String pointerSymbol = getSymbolName(pointerAddress);
                boolean pointerInMemory = isAddressInMemory(pointerAddress);
                String pointerType = "<none>";
                Data targetData = null;
                if (pointerInMemory) {
                    targetData = currentProgram.getListing().getDataAt(pointerAddress);
                    if (targetData != null) {
                        pointerType = targetData.getDataType().getName();
                    }
                }
                String classification = classifyPointerTarget(pointerAddress, pointerInMemory, targetData);
                String notes = pointerInMemory ? "resolved" : "unmapped";
                lines.add(res.targetName + "|" + baseAddress + "|" + String.format("0x%X", offset) + "|" + String.format("0x%X", ptrValue) + "|" + (pointerInMemory ? pointerAddress.toString() : "<none>") + "|" + pointerSymbol + "|" + pointerType + "|" + classification + "|" + notes);
            }
        }
        writeFile(outFile, join(lines, "\n") + "\n");
    }

    private String classifyPointerTarget(Address pointerAddress, boolean pointerInMemory, Data targetData) {
        if (!pointerInMemory) {
            return "unmapped";
        }
        ...
        if (targetData != null) {
            String typeName = targetData.getDataType().getName();
        ...
        if (symbol != null) {
            ...
            if (name.contains("DAT_") || name.contains("ILT_")) {
                return "data_symbol";
            }
            return "symbol";
        }
    ...
    private void writeRecordMemoryFiles(File outDir, List<TargetResolution> resolutions) throws IOException {
        Memory memory = currentProgram.getMemory();
        for (TargetResolution res : resolutions) {
            ...
            String fileName = "record_memory_" + res.targetName + "_" + address.toString().replace(':', '_') + ".txt";
            File outFile = new File(outDir, fileName);
            StringBuilder builder = new StringBuilder();
            builder.append("Record: ").append(res.targetName).append("\n");
            ...
            builder.append("\n\nPointer walk:\n");
            builder.append(dumpPointerWalk(address, POINTER_SCAN_SIZE, memory));
    ...
    private String dumpPointerWalk(Address center, int scanSize, Memory memory) {
        StringBuilder builder = new StringBuilder();
        int pointerSize = currentProgram.getDefaultPointerSize();
        for (int offset = 0; offset + pointerSize <= scanSize; offset += pointerSize) {
            Address probe = center.add(offset);
            if (!isAddressInMemory(probe)) {
                continue;
            }
            byte[] bytes = new byte[pointerSize];
            ...
            Address pointerAddress = toAddr(String.format("0x%X", ptrValue));
            boolean pointerInMemory = isAddressInMemory(pointerAddress);
            String symbol = pointerInMemory ? getSymbolName(pointerAddress) : "<none>";
            String type = "<none>";
            if (pointerInMemory) {
                Data data = currentProgram.getListing().getDataAt(pointerAddress);
                if (data != null) {
                ...
                else if (getFunctionContaining(pointerAddress) != null) {
                    type = "function";
                }
            }
            String classification = classifyPointerTarget(pointerAddress, pointerInMemory, currentProgram.getListing().getDataAt(pointerAddress));
            builder.append(String.format("offset=0x%X pointer=0x%X resolved=%s symbol=%s type=%s class=%s\n",
                offset,
                ptrValue,
                pointerInMemory ? pointerAddress.toString() : "<none>",
                symbol,
                type,
                classification));
        }
